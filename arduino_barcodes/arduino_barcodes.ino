/*
  adapted from https://optogeneticsandneuralengineeringcore.gitlab.io/ONECoreSite/projects/DAQSyncro/BarcodeGeneration/
...
/////////////////USER INPUT///////////////////////////////////
const int TOTAL_TIME = 3000;     // Total time between barcode initiation (includes initialization pulses) in milliseconds. The length of time between one barcode and the next
////////////END USER INPUT///////////////////////////////////
...
const int BARCODE_TIME = 60;  // time for each bit of the barcode to be on/off in milliseconds

const int INITIALIZATION_TIME = 60;  // We warp the beginning and ending of the barcode with 'some signal', well distinct from a barcode pulse, in milliseconds
...
const int TOTAL_BARCODE_TIME = 2 * INITIALIZATION_PULSE_TIME + BARCODE_TIME * BARCODE_BITS; // the total time for the initialization train and barcode signal180960
...
void loop() {
  ...
  digitalWrite(OUTPUT_PIN, LOW);digitalWrite(LED_INTER_PIN, LOW);
  delay(INITIALIZATION_TIME);
  digitalWrite(LED_INTER_PIN, HIGH);digitalWrite(OUTPUT_PIN, HIGH);
  delay(INITIALIZATION_TIME);
  digitalWrite(LED_INTER_PIN, LOW);digitalWrite(OUTPUT_PIN, LOW);
  delay(INITIALIZATION_TIME);
  ...
  for (int i = 0; i < BARCODE_BITS; i++) // for between 0-31 (we will read all 32 bits)
  {
    ...
    {
      digitalWrite(OUTPUT_PIN, HIGH);digitalWrite(LED_INTER_PIN, HIGH);// set the output pin to high
    }
    else
    {
      digitalWrite(OUTPUT_PIN, LOW);digitalWrite(LED_INTER_PIN, LOW);// else set it to low
    }
